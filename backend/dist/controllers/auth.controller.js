...
const login = async (req, res) => {
    try {
        ...
        if (user.mfaEnabled) {
            ...
            const mfaToken = (0, jwt_1.generateAccessToken)({ userId: user.id, email: user.email });
            return res.json({
                success: true,
...
const verifyMFA = async (req, res) => {
    try {
        ...
        let decoded;
        try {
            decoded = (0, jwt_1.verifyRefreshToken)(mfaToken); // Treat mfaToken as normal JWT payload
        }
        catch {
        ...
        const isVerified = (0, mfa_1.verifyMFAToken)(user.mfaSecret, code);
        if (!isVerified) {
...
const setupMFA = async (req, res) => {
    try {
        ...
        const { secret, otpauthUrl } = (0, mfa_1.generateMFASecret)(user.email);
        const qrCodeUrl = await (0, mfa_1.generateQRCodeDataURL)(otpauthUrl);
...
const confirmMFASetup = async (req, res) => {
    try {
        ...
        const isValid = (0, mfa_1.verifyMFAToken)(user.mfaSecret, code);
        if (!isValid) {
