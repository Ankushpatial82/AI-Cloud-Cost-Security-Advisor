...
exports.getAICostSummary = exports.getCostRecommendations = exports.getCostForecast = exports.getCostDashboard = void 0;
...
const getCostDashboard = async (req, res) => {
    try {
        ...
        const costMetrics = await db_1.default.costMetric.findMany({
            where: {
...
exports.getCostDashboard = getCostDashboard;
...
const getCostRecommendations = async (req, res) => {
    try {
        const orgId = req.org?.id;
        if (!orgId)
            return res.status(400).json({ success: false, message: "Organization context required" });
        // Fetch resources with specific sizes or configurations that can be optimized
        const resources = await db_1.default.cloudResource.findMany({
            where: { organizationId: orgId },
            include: {
                account: {
...
exports.getCostRecommendations = getCostRecommendations;
const getAICostSummary = async (req, res) => {
    try {
        ...
        const totalCount = await db_1.default.cloudResource.count({ where: { organizationId: orgId } });
...
Provide action items for immediate savings, prioritizing expensive databases or VMs.`;
        const aiResponse = await (0, openai_1.generateAIExplanation)(prompt, "You are a Cloud FinOps and SaaS cost optimization architect.");
        return res.json({
            success: true,
            data: {
...
exports.getAICostSummary = getAICostSummary;
